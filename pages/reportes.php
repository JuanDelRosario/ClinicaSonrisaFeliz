...
$paidInvoices = getRecords("SELECT f.numero_factura AS Factura, CONCAT(p.nombre,' ',p.apellido) AS Paciente, f.fecha_factura AS Fecha, f.total AS Total FROM facturacion f JOIN pacientes p ON p.id_paciente=f.id_paciente WHERE f.estado_factura='pagada' ORDER BY f.fecha_factura DESC LIMIT 20");
...
<section class="content-section"><h2>Facturas pagadas</h2><?php reportTable($paidInvoices); ?></section>
