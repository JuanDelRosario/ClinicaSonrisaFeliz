...
if ($role === ROLE_DOCTOR) {
    $where = 'WHERE c.id_doctor = ?';
    $params[] = $doctorId;
} elseif ($role === ROLE_RECEPCIONISTA && $search !== '') {
    // Recepción localiza citas rápidamente por el nombre o la cédula del paciente.
    $where = "WHERE (CONCAT(p.nombre, ' ', p.apellido) LIKE ? OR p.cedula LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
}
